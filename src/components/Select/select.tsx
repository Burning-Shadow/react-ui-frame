 ...
 * 
 * ~~~js
 * import { Select } from 'react-ui-frame'
...
export const Select:FC<SelectProps> = (props) => {
  ...
  const containerClass = classNames('ant-select', {
  ...
  return (
    <div className={containerClass} ref={containerRef}>
      <div className="ant-select-input" onClick={handleClick}>
        <Input
          ref={input}
      ...
      <SelectContext.Provider value={passedContext}>
        <Transition
            ...
            timeout={300}
          >
          <ul className="ant-select-dropdown">
            {generateOptions()}
          </ul>
        </Transition>
      </SelectContext.Provider>
      {multiple &&
        <div className="ant-selected-tags" style={{maxWidth: containerWidth.current - 32}}> 
          {
            selectedValues.map((value, index) => {
              return (
                <span className="ant-tag" key={`tag-${index}`}>
                  {value}
