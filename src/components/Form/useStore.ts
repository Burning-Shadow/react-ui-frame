...
function useStore(initialValues?: Record<string, any>) {
  ...
  const getFieldsValue = () => {
    return mapValues(fields, (item: any) => item.value);
  }
  ...
  const resetFields = () => {
    if (initialValues) {
      each(initialValues, (value: any, name: any) => {
        if (fields[name]) {
          dispatch({ type: 'updateValue', name, value });
        }
      });
  ...
  const validateAllFields = async () => {
    ...
    const valueMap = mapValues(fields, (item: any) => item.value);
    // {'username': 'abc'};
    const descriptor = mapValues(fields, (item: any) => transfromRules(item.rules));
    const validator = new Schema(descriptor);
    ...
    try {
      ...
      errors = err.fields;
      each(fields, (value: any, name: any) => {
        // errors 中有对应的 key
        if (errors[name]) {
